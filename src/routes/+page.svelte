...
<Container>
	...
	<Section title="Experience">
		{#each experiences as experience, i}
			<Divider show={i < experiences.length - 1}>
				<Heading level={4} text={experience.company} />
				<Text text={experience.dates} size="xs" color="gray-600" mb="mb-2" />
				<Text text={experience.description} size="xs" color="gray-700" mb="mb-3" />
				{#if experience.achievements.length > 0}
					<List items={experience.achievements} />
				{/if}
			</Divider>
		{/each}
	</Section>
